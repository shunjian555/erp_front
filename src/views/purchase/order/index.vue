<template>
  <div class="page-container">
    ...
    <div class="table-toolbar">
      <div class="toolbar-left"><el-button type="primary" :icon="Plus" @click="handleAdd">{{ $t('purchase.newOrder') }}</el-button><el-button :icon="Download" plain>{{ $t('common.export') }}</el-button></div>
    ...
    <el-dialog v-model="viewVisible" :title="$t('purchase.orderDetail')" width="720px" :close-on-click-modal="false">
      <el-descriptions v-if="viewRow" :column="2" border>
        <el-descriptions-item :label="$t('purchase.orderNo')">{{ viewRow.orderNo }}</el-descriptions-item>
        <el-descriptions-item :label="$t('purchase.orderStatus')"><el-tag :type="statusMap[viewRow.status]?.type || 'info'" size="small">{{ statusMap[viewRow.status]?.label || '—' }}</el-tag></el-descriptions-item>
        ...
        <el-descriptions-item :label="$t('purchase.totalAmount')">{{ formatMoney(viewRow.totalAmount) }}</el-descriptions-item>
        <el-descriptions-item :label="$t('purchase.deliveryDate')">{{ viewRow.deliveryDate }}</el-descriptions-item>
    ...
    <el-dialog v-model="approveVisible" :title="approveTitle" width="500px" :close-on-click-modal="false">
      <el-form :model="approveForm" label-width="100px">
        ...
        <el-form-item :label="$t('purchase.totalAmount')"><el-input :value="formatMoney(approveRow?.totalAmount)" disabled /></el-form-item>
...
const statusMap = computed(() => ({ 0: { label: t('purchase.orderStatusPending'), type: 'warning' }, 1: { label: t('purchase.orderStatusReviewed'), type: 'primary' }, 2: { label: t('purchase.orderStatusPartial'), type: 'success' }, 3: { label: t('purchase.orderStatusCompleted'), type: 'success' }, 4: { label: t('purchase.orderStatusCancelled'), type: 'info' } }))
const searchItems = computed(() => [ { prop: 'orderNo', label: t('purchase.orderNo'), type: 'input' }, { prop: 'supplierName', label: t('purchase.supplier'), type: 'input' }, { prop: 'status', label: t('purchase.orderStatus'), type: 'select', options: Object.entries(statusMap.value).map(([v, l]) => ({ value: Number(v), label: l.label })) } ])
const columns = computed(() => [ { prop: 'orderNo', label: t('purchase.orderNo'), width: 190 }, { prop: 'supplierName', label: t('purchase.supplier'), width: 140 }, { prop: 'totalAmount', label: t('purchase.totalAmount'), width: 130, slot: 'totalAmount', align: 'right' }, { prop: 'quantity', label: t('purchase.quantity'), width: 80, align: 'center' }, { prop: 'status', label: t('purchase.orderStatus'), width: 100, slot: 'status' }, { prop: 'createTime', label: t('purchase.supplierCreateTime'), width: 170 } ])
const formItems = computed(() => [ { prop: 'orderNo', label: t('purchase.orderNo'), type: 'input', span: 12 }, { prop: 'supplierName', label: t('purchase.supplier'), type: 'input', span: 12 }, { prop: 'totalAmount', label: t('purchase.totalAmount'), type: 'number', precision: 2, span: 12 }, { prop: 'deliveryDate', label: t('purchase.deliveryDate'), type: 'date', span: 12 }, { prop: 'remark', label: t('purchase.supplierRemark'), type: 'textarea', rows: 3, span: 24 } ])
const formRules = computed(() => ({ orderNo: [{ required: true, message: t('purchase.inputOrderNo'), trigger: 'blur' }], supplierName: [{ required: true, message: t('purchase.selectSupplier'), trigger: 'blur' }] }))
...
function handleAdd() { dialogTitle.value = t('purchase.newOrder'); Object.keys(formData).forEach(k => formData[k] = ''); formData.id = undefined; dialogVisible.value = true }
function handleView(r) { viewRow.value = r; viewVisible.value = true }
function handleEdit(r) { dialogTitle.value = t('purchase.editOrder'); Object.assign(formData, r); dialogVisible.value = true }
function openApprove(r, approved) { approveRow.value = r; approveForm.approved = approved; approveForm.comment = ''; approveTitle.value = approved ? t('purchase.orderApprove') : t('purchase.orderReject'); approveVisible.value = true }
async function submitApprove() { if (!approveForm.comment.trim()) { ElMessage.warning(approveForm.approved ? t('purchase.approveComment') : t('purchase.rejectReason')); return } approveLoading.value = true; try { await request({ url: '/api/purchase/order/approve', method: 'post', data: { id: approveRow.value.id, approved: approveForm.approved, comment: approveForm.comment } }).catch(() => {}); ElMessage.success(approveForm.approved ? t('purchase.reviewSuccess') : t('purchase.rejectSuccess')); approveVisible.value = false; loadData() } finally { approveLoading.value = false } }
...
async function handleDelete(row) { await ElMessageBox.confirm(t('purchase.deleteOrderConfirm', { no: row.orderNo }), t('header.tips'), { type: 'warning' }); ElMessage.success(t('common.success')); loadData() }
