<template>
  <div class="page-container">
    ...
    <BaseTable :columns="columns" :table-data="tableData" :loading="loading" :total="total" :current-page.sync="queryParams.pageNum" :page-size.sync="queryParams.pageSize" :show-selection="true" :show-index="true" @selection-change="handleSelectionChange" @current-change="handlePageChange" @size-change="handleSizeChange">
      <template #status="{ row }"><BaseStatusTag :type="row.status === 1 ? 'success' : 'warning'">{{ row.status === 1 ? '已完成' : '待确认' }}</BaseStatusTag></template>
      <template #operation="{ row }"><el-button type="primary" link size="small" @click="handleView(row)">查看</el-button></template>
    </BaseTable>
    <BaseDialog v-model="dialogVisible" title="新增入库单" width="650px" :confirm-loading="submitLoading" @confirm="handleSubmit" @cancel="cancelDialog"><BaseForm ref="formRef" v-model="formData" :form-items="formItems" :form-rules="formRules" :col-count="2" /></BaseDialog>
  </div>
</template>
...
const searchItems = [ { prop: 'inboundNo', label: '入库单号', type: 'input' }, { prop: 'orderNo', label: '关联订单', type: 'input' }, { prop: 'warehouse', label: '仓库', type: 'input' } ]
const columns = [ { prop: 'inboundNo', label: '入库单号', width: 190 }, { prop: 'orderNo', label: '关联订单', width: 180 }, { prop: 'supplierName', label: '供应商', width: 130 }, { prop: 'quantity', label: '数量', width: 80, align: 'center' }, { prop: 'warehouse', label: '仓库', width: 100 }, { prop: 'inboundDate', label: '入库日期', width: 120 }, { prop: 'status', label: '状态', width: 90, slot: 'status' } ]
...
const queryParams = reactive({ pageNum: 1, pageSize: 10, inboundNo: '', orderNo: '', warehouse: '' })
const formData = reactive({ id: undefined, inboundNo: '', orderNo: '', warehouse: '', inboundDate: '', remark: '' })

async function loadData() { loading.value = true; try { const res = await (await import('@/utils/request')).default({ url: '/api/purchase/inbound/list', method: 'get', params: queryParams }); tableData.value = res.data.list || []; total.value = res.data.total || 0 } finally { loading.value = false } }
...
function handleView(r) { ElMessage.info(`查看: ${r.inboundNo}`) }
