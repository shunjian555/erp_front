      ...
      <el-row :gutter="12">
        <el-col :span="6">
          <div class="metric-card">
            <div class="metric-label">应付总金额</div>
        ...
        <el-col :span="6">
          <div class="metric-card">
            <div class="metric-label">DPO 周转天数</div>
            <div class="metric-value money primary">{{ avgDays }} 天</div>
            <div class="metric-extra">应付账款周转</div>
          </div>
        </el-col>
      </el-row>
    </div>
    ...
    <el-tabs v-model="activeTab" type="border-card" class="aging-tabs">
      ...
      <el-tab-pane label="供应商统计" name="supplier">
        <BaseTable
          ...
          :show-summary="true"
          :summary-method="supplierSummary"
        >
          <template #totalAmount="{ row }">{{ formatMoney(row.totalAmount) }}</template>
          <template #totalRemain="{ row }">{{ formatMoney(row.totalRemain) }}</template>
        </BaseTable>
      ...
      <el-tab-pane label="明细列表" name="detail">
        ...
        >
          ...
          <template #status="{ row }">
            <el-tag v-if="row.status === 'paid'" type="success" size="small">已付清</el-tag>
            <el-tag v-else-if="row.status === 'partial'" type="warning" size="small">部分付</el-tag>
...
const query = reactive({ pageNum: 1, pageSize: 20, supplierName: '', bucket: '' })
...
const overdueAmount = computed(() => detailData.value.filter(d => d.days > 30).reduce((s, d) => s + d.remainAmount, 0) || 0)
...
const avgDays = computed(() => {
  ...
  return Math.round(detailData.value.reduce((s, d) => s + d.days, 0) / detailData.value.length)
})

const searchItems = [
  { prop: 'supplierName', label: '供应商', type: 'input', span: 8 },
...
const bucketColumns = [
  ...
  { prop: 'totalAmount', label: '应付金额', width: 160, slot: 'totalAmount', align: 'right' },
  { prop: 'totalRemain', label: '未付金额', width: 160, slot: 'totalRemain', align: 'right' },
...
const detailColumns = [
  { prop: 'payableNo', label: '应付单号', width: 150, fixed: true },
  { prop: 'supplierName', label: '供应商', width: 150 },
  { prop: 'orderNo', label: '采购单', width: 150 },
  { prop: 'issueDate', label: '发生日期', width: 110 },
  { prop: 'dueDate', label: '到期日', width: 110 },
  { prop: 'amount', label: '应付金额', width: 130, slot: 'amount', align: 'right' },
  { prop: 'paidAmount', label: '已付金额', width: 130, slot: 'paidAmount', align: 'right' },
  { prop: 'remainAmount', label: '未付金额', width: 130, slot: 'remainAmount', align: 'right' },
...
async function loadData() {
  const [agingRes, supRes] = await Promise.all([
    request({ url: '/api/finance/payable/aging', method: 'get', params: { supplierName: query.supplierName } }),
    request({ url: '/api/finance/payable/supplierStat', method: 'get' })
  ])
  ...
  supplierData.value = (supRes.data.list || []).map(c => ({ ...c, ratio: aging.summary.totalRemain > 0 ? ((c.totalRemain / aging.summary.totalRemain) * 100).toFixed(1) + '%' : '0%' }))
}
...
function handleReset() { query.supplierName = ''; query.bucket = ''; loadData() }
