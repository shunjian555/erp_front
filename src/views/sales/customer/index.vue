<template>
  <div class="page-container">
    ...
    <!-- 详情 -->
    <el-dialog v-model="viewVisible" title="客户详情" width="640px" :close-on-click-modal="false">
      <el-descriptions v-if="viewRow" :column="2" border>
        <el-descriptions-item label="客户名称">{{ viewRow.customerName }}</el-descriptions-item>
        <el-descriptions-item label="联系人">{{ viewRow.contactName }}</el-descriptions-item>
        ...
        <el-descriptions-item label="等级"><el-tag :type="levelMap[viewRow.level]?.type || 'info'" size="small">{{ levelMap[viewRow.level]?.label || viewRow.level }}</el-tag></el-descriptions-item>
        <el-descriptions-item label="状态"><el-tag :type="viewRow.status === 1 ? 'success' : 'info'" size="small">{{ viewRow.status === 1 ? '启用' : '禁用' }}</el-tag></el-descriptions-item>
