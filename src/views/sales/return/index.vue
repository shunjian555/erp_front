...
const searchItems = [ { prop: 'returnNo', label: '退货单号', type: 'input' }, { prop: 'customerName', label: '客户', type: 'input' } ]
const columns = [ { prop: 'returnNo', label: '退货单号', width: 190 }, { prop: 'orderNo', label: '关联订单', width: 180 }, { prop: 'customerName', label: '客户', width: 120 }, { prop: 'amount', label: '金额', width: 120 }, { prop: 'reason', label: '原因', minWidth: 150, showOverflowTooltip: true }, { prop: 'status', label: '状态', width: 90, slot: 'status' }, { prop: 'createTime', label: '创建时间', width: 170 } ]
const formItems = [ { prop: 'returnNo', label: '退货单号', type: 'input', span: 12 }, { prop: 'orderNo', label: '关联订单', type: 'input', span: 12 }, { prop: 'customerName', label: '客户', type: 'input', span: 12 }, { prop: 'amount', label: '金额', type: 'number', precision: 2, span: 12 }, { prop: 'reason', label: '原因', type: 'textarea', rows: 3, span: 24 } ]
...
async function loadData() { loading.value = true; try { const res = await (await import('@/utils/request')).default({ url: '/api/sales/return/list', method: 'get', params: queryParams }); tableData.value = res.data.list || []; total.value = res.data.total || 0 } finally { loading.value = false } }
