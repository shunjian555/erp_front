<template>
  <div class="page-container">
    ...
    <div class="table-toolbar">
      <div class="toolbar-left"><el-button type="primary" :icon="Plus" @click="handleAdd">提交报销</el-button></div>
    ...
    <BaseDialog v-model="dialogVisible" title="提交报销申请" width="600px" :confirm-loading="submitLoading" @confirm="handleSubmit" @cancel="cancelDialog"><BaseForm ref="formRef" v-model="formData" :form-items="formItems" :form-rules="formRules" :col-count="1" /></BaseDialog>
  </div>
</template>
...
const searchItems = [ { prop: 'title', label: '报销标题', type: 'input' }, { prop: 'expenseType', label: '费用类型', type: 'select', options: [{ value: 'travel', label: '差旅费' }, { value: 'office', label: '办公费' }, { value: 'meal', label: '餐饮费' }, { value: 'other', label: '其他' }] } ]
const columns = [ { prop: 'title', label: '报销标题', minWidth: 150 }, { prop: 'expenseType', label: '费用类型', width: 100 }, { prop: 'amount', label: '金额', width: 120 }, { prop: 'description', label: '说明', minWidth: 180, showOverflowTooltip: true }, { prop: 'status', label: '状态', width: 90, slot: 'status' }, { prop: 'createTime', label: '创建时间', width: 170 } ]
const formItems = [ { prop: 'title', label: '标题', type: 'input' }, { prop: 'expenseType', label: '费用类型', type: 'select', options: searchItems[1].options }, { prop: 'amount', label: '金额', type: 'number', precision: 2 }, { prop: 'description', label: '说明', type: 'textarea', rows: 3 } ]
const formRules = { title: [{ required: true, message: '请输入标题', trigger: 'blur' }], amount: [{ required: true, message: '请输入金额', trigger: 'blur' }] }
