<template>
  <div class="page-container">
    ...
    <div class="table-toolbar">
      <div class="toolbar-left"><el-button type="primary" :icon="Plus" @click="handleAdd">新增分类</el-button></div>
    ...
    <BaseTable :columns="columns" :table-data="tableData" :loading="loading" :total="total" :current-page.sync="queryParams.pageNum" :page-size.sync="queryParams.pageSize" :show-index="true" :tree-props="{ children: 'children' }" row-key="id" default-expand-all @current-change="handlePageChange" @size-change="handleSizeChange">
      <template #status="{ row }"><BaseStatusTag :type="row.status === 1 ? 'success' : 'danger'">{{ row.status === 1 ? '启用' : '禁用' }}</BaseStatusTag></template>
...
const searchItems = [ { prop: 'name', label: '分类名称', type: 'input' } ]
const columns = [ { prop: 'name', label: '分类名称', width: 200 }, { prop: 'code', label: '编码', width: 120 }, { prop: 'sort', label: '排序', width: 80, align: 'center' }, { prop: 'status', label: '状态', width: 90, slot: 'status' }, { prop: 'createTime', label: '创建时间', width: 170 } ]
const formItems = [ { prop: 'name', label: '分类名称', type: 'input' }, { prop: 'code', label: '编码', type: 'input' }, { prop: 'sort', label: '排序', type: 'number' }, { prop: 'parentId', label: '上级分类', type: 'select', options: [] }, { prop: 'status', label: '状态', type: 'radio', options: [{ value: 1, label: '启用' }, { value: 0, label: '禁用' }] } ]
...
function handleAdd() { dialogTitle.value = '新增分类'; Object.keys(formData).forEach(k => formData[k] = ''); formData.id = undefined; dialogVisible.value = true }
function handleEdit(r) { dialogTitle.value = '编辑分类'; Object.assign(formData, r); dialogVisible.value = true }
